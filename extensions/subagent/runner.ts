/**
 * Subagent runner: spawns isolated `pi` processes, parses their JSON event
 * stream, and threads progress updates back. Extracted from `index.ts` —
...
export async function runSingleAgent(
	...
	try {
		...
		const exitCode = await new Promise<number>((resolve) => {
			...
			const existingTrail = process.env.PI_SUBAGENT_TRAIL ?? "";
			const newTrail = existingTrail ? `${existingTrail},${agentName}` : agentName;
			const proc = spawn(invocation.command, invocation.args, {
				...
				env: {
					...process.env,
					PI_SUBAGENT_DEPTH: String((currentDepth ?? 0) + 1),
					PI_SUBAGENT_TRAIL: newTrail,
				},
			});
			let buffer = "";
