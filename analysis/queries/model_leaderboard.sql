-- Model leaderboard with conservative CI-based composite ranking.
-- Groups by model + thinking level (experiment assignments collapsed).
-- Uses Wilson lower bounds for proportions and mean lower CIs for satisfaction.
SELECT
  ...
  ROUND(AVG(CASE WHEN verification_total_count > 0 THEN 1.0 ELSE 0.0 END), 3) AS verification_adoption_rate,
